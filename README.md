...
The command fingerprints and extracts the sealed ZIP, reconciles structure and technical-logic record kinds, parses all parseable records, binds identifiers within their owning transformation using native field metadata, discovers primary validation endpoints from selected mapping invocations, emits source-neutral candidate rules and evidence, generates coverage/construct/ambiguity/helper reports, performs conservative render→parse→bind→normalise byte round-trips where verification is not externally blocked, and exits non-zero on a run-level gate failure.
...
The ten selected mappings contain 128 `rule_*` reusable invocations. The R00 bounded-outcome test currently finds 106 invocations with exactly one primary outcome endpoint, 21 helper/non-outcome invocations, and one semantic ambiguity (`rule_Standard_Lead_Time`, with ARIBA and COMM endpoints). The ambiguity is emitted, not guessed. Of the 106 single-primary rules, 79 are fully bound/parse-complete and 27 remain partial because one or more identifiers cannot be resolved within evidence-backed scope. Eleven rules are currently eligible for the expression-only round-trip oracle and all 11 pass; the other 95 remain `blocked_external`.
